//
//  News.swift
//  EarthValley80
//
//  Created by LeeJiSoo on 2022/10/27.
//

import UIKit

struct News {
    let newsTitle: String
    let newsCategory: String
}

//TODO: - 더미 데이터, 나중에 지우겠습니다.
var data: [News] = [
    News(newsTitle: "1인류보다 로봇 진화 속도가 더 빠르대요, 청소로봇은 '루시'…생각하는 로봇 등장", newsCategory: "시사"),
    News(newsTitle: "2인류보다 로봇 진화 속도가 더 빠르대요, 청소로봇은 '루시'…생각하는 로봇 등장", newsCategory: "시사"),
    News(newsTitle: "3인류보다 로봇 진화 속도가 더 빠르대요, 청소로봇은 '루시'…생각하는 로봇 등장", newsCategory: "시사"),
    News(newsTitle: "3인류보다 로봇 진화 속도가 더 빠르대요, 청소로봇은 '루시'…생각하는 로봇 등장", newsCategory: "시사"),
    News(newsTitle: "3인류보다 로봇 진화 속도가 더 빠르대요, 청소로봇은 '루시'…생각하는 로봇 등장", newsCategory: "시사"),
    News(newsTitle: "3인류보다 로봇 진화 속도가 더 빠르대요, 청소로봇은 '루시'…생각하는 로봇 등장", newsCategory: "시사"),
    News(newsTitle: "1급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "2급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사"),
    News(newsTitle: "3급락한 카카오뱅크 주식, 카뱅이 사들이는 이유는", newsCategory: "시사")
    ]
